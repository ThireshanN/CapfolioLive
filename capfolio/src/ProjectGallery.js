...
const ProjectGallery = () => {
  ...
  const displayedProjects = (isFiltered ? filteredProjects : projects).slice(
    startIndex,
    endIndex
  );
  ...
  const fetchSearchedProject = async (body) => {
    console.log(body);
    const response = await fetch("project/searchProject?keyword=" + body);
    ...
    try {
      ...
      if (responseData.message === "No projects can be found for the given search phrase") {
        setIsNoResults(true);
      } else {
