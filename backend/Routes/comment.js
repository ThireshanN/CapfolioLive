...
commentRouter.get('/getComments', async (req, res) => { 
    try {
        const sql = "SELECT CommentDesc, FirstName, lastName, SUBSTRING(createdTime, 1, 10) AS createdTime, ProjectName FROM Comment INNER JOIN Users ON Comment.UserID_FK = Users.UserID  INNER JOIN Project ON Comment.ProjectID_FK = Project.ProjectID ORDER BY CommentID DESC;";
